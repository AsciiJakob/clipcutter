...
const char* const EXPORT_AS_OPTIONS_STRS[] = {
    "Export as Video",
    "Export as Audio"
};
...
char* recieveAndProcessFramesVideo(
    ...
    if (exportState->exportOptions.exportAsComboIndex == EXPORT_AS_OPTION_AUDIO) {
...
ExportError* remuxClip(MediaClip* mediaClip, ExportState* exportState) {
...

    {
        ...
        if (enabledAudioStreamCount != 0) {
            if (exportState->exportOptions.exportAsComboIndex == EXPORT_AS_OPTION_VIDEO) {
                ...
                audioEnc = (AVCodec*) avcodec_find_encoder(AV_CODEC_ID_MP3);            
            }
            ...
            audio_fifo = av_audio_fifo_alloc(
                ...
                audioEncCtx->ch_layout.nb_channels,
        ...
        if (enabledAudioStreamCount == 0) {
            ...
            int sample_rates[] = { audioEncCtx->sample_rate, -1 };
        ...
        if (shouldExportVideo) {
            ...
            av_opt_set(videoEncCtx->priv_data, "preset", ENCODER_PRESETS[expOpts->encoderPresetComboIndex], 0);
...

    {
        ...
        while (1) {
            ...
            // also checked in recieveAndProcessFramesVideo() for when we are
            // flushing, but checking here optimizes export more for main export loop.
            if (!shouldExportVideo) {
                if (in_stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
                    continue;
                }
            ...
            if (in_index == inVideoStreamIdx) {
                ...
                if (idx != -1) {
                    ...
                    while (1) {
                        ...
                        // old rebasing pts code from before switching to fifo
                        # if 0
                        ...
                        if (ret < 0) {
                            err = alloc_error("error sending frame to fifo");
                            goto cleanup;
                        }
